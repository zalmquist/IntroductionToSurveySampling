...
plot(coots$CLUTCH,coots$VOLUME,pch=19,cex=.5)
@
\end{center}
\end{frame}
...
one<-sapply(foo,"[",1)
two<-sapply(foo,"[",2)
plot(1:length(o),one[o],pch=19,cex=.1)
points(1:length(o),two[o],pch=19,cex=.1)

for(i in 1:length(o))
segments(x0=i,x1=i,y0=one[o[i]],y1=two[o[i]],cex=1)
@
\end{center}
\end{frame}
...
cluster.table<-data.frame(Clutch=1:184,Mi,ybari,si2,hat.ti,var1,var2)
head(cluster.table)
@
\end{frame}
...
<<>>=
M0<-255077536
totalCounty<-data.frame(
state=statepop$STATE, 
county=statepop$COUNTY,
popsize=statepop$POPN,
psi=statepop$POPN/(M0),
numPhys=statepop$PHYS,
...
<<>>=
### Table Descriptives
sum(totalCounty$ti_psi)
sd(totalCounty$ti_psi)/sqrt(100)
